...
if __name__ == "__main__":
    ...
    parser.add_argument('--MINOVERLAP', default=0.5, type=float, help='Use to specify the map 0.x you want to get')
    parser.add_argument('--split', default='test', type=str, help="dataset split in ['val', 'test']")
    parser.add_argument('--VOCdevkit_path', default="VOCdevkit", help='Pretrained Model Path')
    parser.add_argument('--confidence', default=0.001, type=float, help='Pretrained Model Path')
    parser.add_argument('--output_file', default="./map_out/bonus2.pkl", help='PKL for evaluation')
    parser.add_argument('--score_threhold', default=0.5, type=float, help='Confidence threshold for positive prediction')    # 0.3
    parser.add_argument('--nms_iou', default=0.5, type=float, help='Threshold for non maximum suppression')  #0.5
    parser.add_argument('--image_size', default=640, type=int, help='Image Size')
    args = parser.parse_args()
    predictions = defaultdict(list)

    image_idss = open(os.path.join(VOCdevkit_path, "VOC2007/ImageSets/Main/val.txt")).read().strip().split()
    image_ids = []
    for image_id in image_idss:
        if image_id.endswith('.jpg'):
            image_ids.append(image_id[-10:-4])
    #  print(image_ids)
    ...
    if map_mode == 0 or map_mode == 1:
        ...
        for image_id in tqdm(image_ids):
            image_path = image_path = os.path.join(VOCdevkit_path, "VOC2007/JPEGImages/test/image/" + image_id + ".jpg")  # test
            image = Image.open(image_path)
            if map_vis:
                image.save(os.path.join(map_out_path, "images-optional/" + image_id + ".jpg"))
            print(image_id)
            result = yolo.get_map_txt(image_id, image, class_names, map_out_path)
    ...
    val_annotation_path = './VOCdevkit/VOC2007/ImageSets/Main/val.txt'   # test
